...
template<class CT>
void ofxColorGradient<CT>::loadFromImage(const ofPixels & img, int skipCount) {
	if(skipCount < 1)
		skipCount = 1;
	int w = img.getWidth();
	int h = img.getHeight();
...
template<class CT>
void ofxColorGradient<CT>::setupAsTurbo(int numSamples){

	if(numSamples < 2) return;

	reset();
	auto tg = createTurboGradient();

	for(int i = 0; i < numSamples; i++){
		float pct = i / float(numSamples - 1);
		CT color = tg.getColorAtPercent(pct);
		addColor(color);
	}
}
...
template<>
ofxColorGradient<ofColor> ofxColorGradient<ofColor>::createTurboGradient(){

	ofxColorGradient<ofColor> g;
	...
	for(int i = 0; i < 256; i++){
		g.addColor(ofColor(turbo_srgb_bytes[i][0], turbo_srgb_bytes[i][1], turbo_srgb_bytes[i][2]));
	}
	return g;
}
...
template<>
ofxColorGradient<ofFloatColor> ofxColorGradient<ofFloatColor>::createTurboGradient(){

	ofxColorGradient<ofFloatColor> g;
	// Copyright 2019 Google LLC.
	// SPDX-License-Identifier: Apache-2.0
	// Author: Anton Mikhailov
	// The look-up tables contains 256 entries. Each entry is a an sRGB triplet.
	...
	for(int i = 0; i < 256; i++){
		g.addColor(ofFloatColor(turbo_srgb_floats[i][0], turbo_srgb_floats[i][1], turbo_srgb_floats[i][2]));
	}
	return g;
}
...
template<class CT>
void ofxColorGradient<CT>::createCache(int numSamples) {
...
template<class CT>
bool ofxColorGradient<CT>::replaceColorAtIndex(int index, CT newColor) {
...
template<class CT>
void ofxColorGradient<CT>::drawDebug(float x, float y, float w, float h, int numSteps, bool bDrawDirFlip, bool bDrawVertical) const{
	...
	for(float i = 0; i <= numSteps; i++) {
		...
		if (!bDrawDirFlip){
			...
			color = getColorAtPercent(1.0f - pct);
		}
...
template<class CT>
CT ofxColorGradient<CT>::getColorAtPercent(float pct) const{
	...
	const CT &leftC = gradientBar[leftColor];
	const CT &rightC = gradientBar[rightColor];
...
template<class CT>
CT & ofxColorGradient<CT>::getColorCacheAtPercent(float pct){
...
template<class CT>
CT & ofxColorGradient<CT>::getColorCacheAtPercent_01(float pct){
...
template class ofxColorGradient<ofColor>;
template class ofxColorGradient<ofFloatColor>;
